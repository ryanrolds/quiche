...
module.exports = {
  'createUrl': function(chart, secure) {
    ...
    return ((secure) ? 'https://' : 'http://') + getUrl + qs;
  },
...
function processTitle(chart) {
  ...
  params.chts = compact([title.color, title.size, title.align]).join(',');
...
function processBarWidthSpacing(chart) {
  if(chart.getType() === 'bar') {
    return {
      'chbh': compact(['a', chart.barSpacing, chart.barWidth]).join(',')
    };
  }
...
function processLegend(chart) {
  ...
  if(legend.position || legend.order) {
    params.chdlp = compact([legend.position || 'r', legend.order || 'l']).join('|');
  }
  ...
  if(legend.color || legend.size) {
    ...
    params.chdls = compact([legend.color, legend.size]).join(',');
  }
